...
class ResetPasswordController @Inject() (
  ...
  def view(token: UUID) = silhouette.UnsecuredAction.async { implicit request: Request[AnyContent] =>
    authTokenService.validate(token).map {
      ...
      case None => Redirect(routes.CompanyController.shopView()).flashing("error" -> Messages("invalid.reset.link"))
    }
  }
          ...
          password => userService.retrieve(authToken.userID).flatMap {
            case Some(user) if user.providerID == CredentialsProvider.ID =>
              ...
              authInfoRepository.update[PasswordInfo](LoginInfo (user.providerID.getOrElse(""), user.providerKey.getOrElse("")), passwordInfo).map { _ =>
                Redirect(routes.CompanyController.shopView()).flashing("success" -> Messages("password.reset"))
              }
            case _ => Future.successful(Redirect(routes.CompanyController.shopView()).flashing("error" -> Messages("invalid.reset.link")))
          })
      case None => Future.successful(Redirect(routes.CompanyController.shopView()).flashing("error" -> Messages("invalid.reset.link")))
    }
  }
}
