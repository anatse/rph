...
В приложении реализованы следующие свойства
...
Для UI используется библиотека scala-js. На текущий момент в состав приложения включено два проекта со scala-js. Они различаются по назначению страниц. Один проект дл яклиентской части, другой для администрирования 
...
В проекте настроена автосорка с помощью [travis](https://travis-ci.org). Описание находится в файле .travis.yml
Для рподсчета покрытия Unit тестами используется [codecoverage](https://codecov.io)

## Сборка и запуск

Перед запуском необходимо установить и запустить базу данных mongodb. Для этого можно либо установить лдокально либо запустить чрез docker.
...
Поеле этого сайт доступен поссылке [http://localhost:9000]()http://localhost:9000)
